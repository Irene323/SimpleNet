...
class Config():
    ...
    pkl_dir = '../out/net2_tanh_margin11.3137/net2_tanh_margin11.3137_merge_200000_train_epoch40.pkl'
    write_bool = True
...
class TripletLoss(torch.nn.Module):
    ...
    def forward(self, anchor, positive, negative, size_average=True):
        ...
        # if (anchor - positive).pow(2).sum(1).min()<=0 or (anchor - negative).pow(2).sum(1).min()<=0:
        # print((anchor - positive).pow(2).sum(1))
        # print((anchor - negative).pow(2).sum(1))
        ...
        for i, data in enumerate(train_dataloader, 0):
            ...
            if np.isnan(loss_one):
                torch.save(net.state_dict(),
                           (Config.write_dir.rstrip('epoch{}.pkl')+'loss2.jpg').format(Config.margin, Config.margin))
            ...
            if (i == (int((Config.patch_len-1) / Config.batchsize))):  # ((i % Config.patch_len) == (Config.patch_len - 1)):
                ...
                print("Epoch number {}\n Current loss {}\n".format(epoch+40, loss / Config.patch_len))
                counter.append(iteration_number)
                loss_history.append(loss / Config.patch_len)
                if Config.write_bool and (epoch%10)==0:
                    torch.save(net.state_dict(), (Config.write_dir.format(Config.margin, Config.margin, epoch+40)))
